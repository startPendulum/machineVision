...
# 对right平移后进行透视变换，得到的图像会向匹配的方向移动，为了使全部图像显示出来，需要先对原图像进行平移
new_left = cv2.warpPerspective(left, M.dot(H), (x_max - x_min, y_max - y_min))
new_left[-y_min:-y_min + h2, -x_min:-x_min + w2] = right
